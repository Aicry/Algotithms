...
public:
   ...
   //层序遍历
    ...
    //是否为叶子节点
    bool isExternalNode(node* root){
        if(root->_left==NULL&&root->_right==NULL)return true;
        return false;
    }
    ...
    void eulerTour(){
     ...
     void _eulerTour(node* root){
         if(root==NULL)return;
         if(isExternalNode(root))cout<<" "<<root->_data<<" ";
         else {
             cout<<"(";
             _eulerTour(root->_left);
             cout<<root->_data;
             _eulerTour(root->_right);
              cout<<")";
         }
     }
    //求叶子节点的个数
...
void test_binary_tree()
{
    ...
    */
    ...
    char array[]={'-','/','*','+','3','#','#','1','#','#','3','#','#','+','-','9','#','#','5','#','#','2','#','#',
    '+','*','3','#','#','-','7','#','#','4','#','#','6','#','#'};
    binary_tree<char> t(array, sizeof(array) / sizeof(char), '#');
    t.prev_order();
    t.prev_order_no_r();
    t.in_order();
    t.in_order_no_r();
